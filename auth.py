...
AUTH0_DOMAIN = os.environ.get('AUTH0_DOMAIN', 'yangcliff.auth0.com')
ALGORITHMS = ["RS256"]
...
def get_token_auth_header():
    ...
    elif len(header_parts) == 1 or len(header_parts) > 2:
        raise AuthError({
            "code": "invalid_header",
...
def check_permissions(permission, payload):
    if "permissions" not in payload:
        raise AuthError({
            "code": "invalid_claims",
            "descriptions": "Permissions not included in payload"
        }, 400)
...
def verify_decode_jwt(token):
    ...
    # header data
    unsverified_header = jwt.get_unverified_header(token)
